...
def cptac_table():
	...
	cdegron_df = pd.read_csv('/workspace/users/msanchezg/notebooks/cdegron_wtnsfs_cptac.tsv', sep = '\t')
	...
	aa_df = pd.read_csv(r'/workspace/users/msanchezg/notebooks/last_aa_cptac.tsv', sep = '\t')
	...
	prots_df = pd.read_csv("/workspace/users/msanchezg/notebooks/protein_positions_relative_allmut_cptac.tsv", sep='\t')
	prots_df['Protein_position_relative'] = pd.cut(prots_df.Protein_position2,bins=[0,25,50,75,100],labels=['Q1','Q2','Q3','Q4'])
...
def ccle_table():
	...
	################################################################
	print('Load cdegron table and merge')
	cdegron_df = pd.read_csv('/workspace/users/msanchezg/notebooks/cdegron_wtnsfs_ccle.tsv', sep = '\t')
	...
	################################
	print('Load last_aa table and merge')
	aa_df = pd.read_csv('/workspace/users/msanchezg/notebooks/last_aa_ccle.tsv', sep = '\t')
	...
	###################
	print('Load protein_positions_relative table and merge')
	prots_df = pd.read_csv("/workspace/users/msanchezg/notebooks/protein_positions_relative_ccle.tsv", sep='\t')
	prots_df['Protein_position_relative'] = pd.cut(prots_df.Protein_position2,bins=[0,25,50,75,100],labels=['Q1','Q2','Q3','Q4'])
	prots_df = prots_df.drop_duplicates(keep='first')
	prots_df = prots_df[['ID','Protein_position2','Protein_position_relative']]
